...
void next_change() {
    int now, next, tot, mph;
    string str;

    mph = HOUR / MINUTE;
    now = query_hour(time())*mph + query_minutes(time());
    switch(__TimeOfDay) {
        case "night":
            str = "sunrise";
            break;
    ...
    next = (mph*__Almanac[str][0]) + __Almanac[str][1];
    ...
    __Events[file_name(this_object())] =
      (["time":time()+(MINUTE*tot),"function":"change_sky","args":({}),"regular":0]);
}
