...
string *query_quests();
private int register_channels();
private int setup_inform();
...
void set_rname(string rname) {
    if(previous_object() && base_name(previous_object()) == "/adm/obj/login") {
	real_name = rname;
	return;
    }
...
void set_name(string str) {
    ...
    // Login often holds path-based euid until creator_file uids are wired;
    // still allow the login object to name the player.
    if(previous_object() && base_name(previous_object()) == "/adm/obj/login") {
	::set_name(str);
	return;
    }
    euid = geteuid(previous_object());
    if(euid != UID_ROOT && euid != UID_USERACCESS) return;
    ::set_name(str);
}
